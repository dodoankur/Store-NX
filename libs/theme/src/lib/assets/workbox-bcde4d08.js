define("./workbox-bcde4d08.js", ["exports"], function (e) {
  "use strict"
  try {
      ...
      if ((!o && this.s && (o = this.s), o)) {
        ...
        return (
          i instanceof Promise &&
            this.i &&
            (i = i.catch(s => this.i.handle({ url: n, request: e, event: t }))),
          i
        )
      }
    }
  ...
  const h = {
    ...
    l = e => [h.prefix, e, h.suffix].filter(e => e && e.length > 0).join("-"),
    f = e => e || l(h.precache),
    w = e => e || l(h.runtime),
    p = new Set()
  const d = (e, t) => e.filter(e => t in e),
    ...
    q = g,
    R = async ({ request: e, fetchOptions: t, event: s, plugins: r = [] }) => {
      if (
  ...
  class N {
      ...
      for (const s of e) {
        ...
        if ("string" != typeof s && s.integrity) {
          if (this.l.has(e) && this.l.get(e) !== s.integrity)
            throw new n("add-to-cache-list-conflicting-integrities", { url: r })
    ...
    async p({
      ...
      let a,
        u = await R({ event: r, plugins: i, request: c })
  ...
  ;(e.NetworkFirst = class {
    constructor(e = {}) {
      if (((this.o = w(e.cacheName)), e.plugins)) {
        ...
        this.g = t ? e.plugins : [v, ...e.plugins]
      } else this.g = [v]
      ;(this.m = e.networkTimeoutSeconds || 0),
        (this.q = e.fetchOptions),
    ...
    async handle({ event: e, request: t }) {
      ...
      let i
      if (this.m) {
        const { id: n, promise: o } = this.v({ request: t, event: e, logs: s })
        ;(i = n), r.push(o)
      }
      const o = this.U({ timeoutId: i, request: t, event: e, logs: s })
      r.push(o)
      let c = await Promise.race(r)
      if ((c || (c = await o), !c)) throw new n("no-response", { url: t.url })
      return c
    }
    v({ request: e, logs: t, event: n }) {
      let s
      return {
        promise: new Promise(t => {
          s = setTimeout(async () => {
            t(await this.L({ request: e, event: n }))
          }, 1e3 * this.m)
        }),
        id: s,
      }
    }
    async U({ timeoutId: e, request: t, logs: n, event: s }) {
      let r, i
      try {
        i = await R({
          request: t,
      ...
      if ((e && clearTimeout(e), r || !i))
        i = await this.L({ request: t, event: s })
      else {
        const e = i.clone(),
          n = m({
            cacheName: this.o,
    ...
    (e.NetworkOnly = class {
      constructor(e = {}) {
        ;(this.g = e.plugins || []), (this.q = e.fetchOptions)
      }
      async handle({ event: e, request: t }) {
        let s, r
        "string" == typeof t && (t = new Request(t))
        ...
        if (!r) throw new n("no-response", { url: t.url, error: s })
        return r
      }
    }),
