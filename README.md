...
This has little bit of bugs in here, but admin, server works very fine! To
start, You need to have cli, `yarn global add nx` or `npm i -g nx`

And also install deps with, `yarn` or `npm i`

you can first start server with, `nx run server:serve`

And now you can run admin with, `nx run admin:serve`

To start the store, You first need to build the store with, `nx run store:build`
and then start ssr-server with, nx run store-ssr:serve However also note store
have some bugs.
